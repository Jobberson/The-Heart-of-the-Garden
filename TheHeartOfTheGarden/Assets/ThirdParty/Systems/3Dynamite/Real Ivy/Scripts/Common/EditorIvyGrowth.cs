...
//				Refine (infoPool.ivyContainer.branches [b]);
		...
		void CheckGrabPoint(BranchContainer branch){
			for (int i = 0; i < 6; i++) {
				...
				Ray ray = new(branch.branchPoints [branch.branchPoints.Count - 1].point + branch.growDirection * infoPool.ivyParameters.stepSize, Quaternion.AngleAxis(angle, branch.growDirection) * branch.GetLastBranchPoint().grabVector);
				RaycastHit RC;
