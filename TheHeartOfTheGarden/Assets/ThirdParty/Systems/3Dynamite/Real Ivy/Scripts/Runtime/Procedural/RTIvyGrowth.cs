...
namespace Dynamite3D.RealIvy
{
	public class RuntimeIvyGrowth
	{
		...
		void CheckWall(RTBranchContainer branch)
		{
			...
			Ray ray = new(branch.branchPoints[branch.branchPoints.Count - 1].point, direction);
			RaycastHit RC;
		...
		{
			Ray ray = new(potentialPoint.point, -oldSurfaceNormal);
			RaycastHit RC;
		...
		{
			Ray ray = new(potentialPoint.point + branch.branchPoints[branch.branchPoints.Count - 1].grabVector * 2f * branch.currentHeight, -branch.growDirection);
			RaycastHit RC;
			if (Physics.Raycast(ray, out RC, ivyParameters.stepSize * 1.15f, ivyParameters.layerMask.value))
			{
		...
		{
			Ray ray = new(branch.branchPoints[branch.branchPoints.Count - 1].point, branch.growDirection);
			RaycastHit RC;
			if (!Physics.Raycast(ray, out RC, ivyParameters.stepSize * 1.15f, ivyParameters.layerMask.value))
			{
				if (Random.value < ivyParameters.grabProvabilityOnFall)
		...
		void CheckGrabPoint(RTBranchContainer branch)
		{
			for (int i = 0; i < 6; i++)
			{
				...
				Ray ray = new(branch.branchPoints[branch.branchPoints.Count - 1].point + branch.growDirection * ivyParameters.stepSize, Quaternion.AngleAxis(angle, branch.growDirection) * branch.GetLastBranchPoint().grabVector);
				RaycastHit RC;
