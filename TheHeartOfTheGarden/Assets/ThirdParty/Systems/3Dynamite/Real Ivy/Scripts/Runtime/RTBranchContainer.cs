...
namespace Dynamite3D.RealIvy
{
	public class RTBranchContainer
	{
			...
			GameObject ivyGO, RTMeshData[] leavesMeshesByChosenLeaf)
		{
			...
			{
				RTBranchPoint rtBranchPoint = new(branchContainer.branchPoints[i], this);
			...
			if (ivyParameters.generateLeaves)
			{
				...
				for (int i = 0; i < branchPoints.Count; i++)
				{
					...
					{
						RTLeafPoint rtLeafPoint = new(leavesToBake[j], ivyParameters);
		...
		{
			Vector2 res = new(totalLength * ivyParameters.uvScale.y + ivyParameters.uvOffset.y,
