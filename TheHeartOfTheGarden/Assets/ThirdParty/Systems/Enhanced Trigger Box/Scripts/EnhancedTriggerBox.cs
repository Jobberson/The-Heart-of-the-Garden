...
namespace EnhancedTriggerbox
{
    ...
    public class EnhancedTriggerBox : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField]
        private List<EnhancedTriggerBoxComponent> conditions = new();

        /// <summary>
        /// Holds all the responses the user has added to the trigger box
        /// </summary>
        [SerializeField]
        private List<EnhancedTriggerBoxComponent> responses = new();

        /// <summary>
        /// Holds all the exit responses the user has added to the trigger box
        /// </summary>
        [SerializeField]
        private List<EnhancedTriggerBoxComponent> exitResponses = new();
        ...
        private void DisplayAddComponent(string componentType, string[] componentNames, List<EnhancedTriggerBoxComponent> componentList)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                if (conditionIndex != 0)
                {
                    ...
                    if (conType == null)
                    {
                        Debug.Log("Unable to find the " + componentType + " " + componentNames[conditionIndex].Replace(" ", "").ToString() + ". Make sure it has the EnhancedTriggerbox.Component namespace.", this);
                    }
                    else
                    {
        ...
        void Start()
        {
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(followTransformName))
                {
                    ...
                    {
                        Debug.Log("Unable to find game object" + followTransformName + " for Trigger Follow. Reverting to static.", this);
        ...
        private IEnumerator ConditionsMet()
        {
            ...
            if (debugTriggerBox)
            {
                Debug.Log(gameObject.name + " has been triggered!", this);
            }
        ...
        private void OnTriggerEnter(Collider other)
        {
            if (!disableEntryCheck)
            {
                if (triggerTags == "Player")
                {
                    if (other.TryGetComponent(out PlayerTriggerDetector _))
                    {
                        Debug.Log("Player entered dialogue trigger area.", this);
                        triggered = true;
        ...
        /// <summary>
        /// Called every frame while a collider remains inside this trigger.
        /// Keeps the 'triggered' state and collidingObject current so conditions
        /// that become true while the player is inside the box are detected.
        /// </summary>
        private void OnTriggerStay(Collider other)
        {
            if (!disableEntryCheck)
            {
                // If tag filtering uses special "Player" token and a player detector component:
                if (triggerTags == "Player")
                {
                    if (other.TryGetComponent(out PlayerTriggerDetector _))
                    {
                ...
                // If no tag filter or tags include this object's tag
                if (string.IsNullOrEmpty(triggerTags) || (triggerTags.Split(',').Contains(other.gameObject.tag)))
                {
                    triggered = true;
                    collidingObject = other.gameObject;
                }
